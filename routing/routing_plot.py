...
def plot_single(ax, results_df):
    ...
    ax.plot(x, mean_p, marker="o", color=COLOR_BASE, label="The Draw Router", linestyle="--")
    ax.fill_between(x, mean_p - std_p, mean_p + std_p, color=COLOR_BASE, alpha=FILL_ALPHA)
...
plot_single(ax2, results_base_df)
fig2.tight_layout()
fig2.savefig("plot_base.pdf", dpi=150)
fig2.savefig("plot_base.png", dpi=150)
plt.show()
...
# ── Figure 4: English vs Polish overall accuracy ───────────────────────────────
lang_df      = pd.read_parquet("router_results_by_language.parquet")
lang_base_df = pd.read_parquet("router_base_results_by_language.parquet")

x = np.arange(len(ALPHA_TICKS))

def plot_language_overall(ax, lang, title):
    vals_pref = lang_df.loc[lang]      * 100
    vals_base = lang_base_df.loc[lang] * 100

    ax.plot(x, vals_pref, marker="o", color=COLOR_PREF, linewidth=2,
            label="Group-aware")
    ax.plot(x, vals_base, marker="s", color=COLOR_BASE, linewidth=1.5,
            linestyle="--", label="No-Groups")
    ax.plot([x[0], x[-1]], [vals_base.iloc[0], vals_base.iloc[-1]],
                    color='grey', linestyle=":", linewidth=1.0, alpha=0.5, zorder=1)

    ax.set_xticks(x); ax.set_xticklabels(ALPHA_TICKS)
    ax.set_xlabel("Budget α"); ax.set_ylabel("Accuracy (%)")
    ax.set_title(title)
    ax.legend(fontsize=8); ax.grid(axis="y", linestyle=":", alpha=0.6)

fig4, axes4 = plt.subplots(1, 2, figsize=(12, 4), sharey=True)
plot_language_overall(axes4[0], "en", "Routing Accuracy – English")
plot_language_overall(axes4[1], "pl",  "Routing Accuracy – Polish")

fig4.suptitle("Routing Accuracy by Language", fontsize=12, fontweight="bold")
fig4.tight_layout()
fig4.savefig("plot_by_language.pdf", dpi=150)
fig4.savefig("plot_by_language.png", dpi=150)
plt.show()
...
fig6.suptitle("Notable Subjects: Largest Group-Aware vs No-Group Gap",
