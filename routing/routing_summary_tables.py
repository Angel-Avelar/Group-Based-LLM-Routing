...
subj_lang_df        = pd.read_parquet("router_results_by_subject_language.parquet")
subj_lang_base_df   = pd.read_parquet("router_base_results_by_subject_language.parquet")
...
# ── By language ───────────────────────────────────────────────────────────────
lang_combined = pd.concat([
    grouped_mean_to_display(lang_df).add_suffix(""),
    grouped_mean_to_display(lang_base_df).add_suffix(""),
], keys=["Preference-aware", "Base"])
lang_combined.index = lang_combined.index.map(lambda x: f"{x[0]} – {x[1]}")

latex_lang = to_latex(
    lang_combined,
    caption="Routing accuracy (\\%) by language. Averaged over 500 runs.",
    label="tab:accuracy_by_language",
)
...
# ── By subject x language ─────────────────────────────────────────────────────
subj_lang_combined = pd.concat([
    grouped_mean_to_display(subj_lang_df),
    grouped_mean_to_display(subj_lang_base_df),
], keys=["Preference-aware", "Base"])
subj_lang_combined.index = subj_lang_combined.index.map(
    lambda x: f"{x[0]} – {x[1][0]}, {x[1][1]}" if isinstance(x[1], tuple) else f"{x[0]} – {x[1]}"
)

latex_subj_lang = to_latex(
    subj_lang_combined,
    caption="Routing accuracy (\\%) by subject and language. Averaged over 500 runs.",
    label="tab:accuracy_by_subject_language",
)

# ── Print all ─────────────────────────────────────────────────────────────────
for name, latex in [
    ("Overall",            latex_overall),
    ("By Language",        latex_lang),
    ("By Subject",         latex_subj),
    ("By Subject×Language", latex_subj_lang),
]:
    print(f"\n% {'─'*60}")
    print(f"% {name}")
    print(f"% {'─'*60}")
...
with open("routing_tables.tex", "w") as f:
    ...
    for latex in [latex_overall, latex_lang, latex_subj, latex_subj_lang]:
        f.write(latex + "\n\n")
