...
for alpha in alphas:
    ...
    by_lang[alpha_label]           = []
    by_lang_base[alpha_label]      = []
    by_subj[alpha_label]           = []
    by_subj_base[alpha_label]      = []
    by_subj_lang[alpha_label]      = []
    by_subj_lang_base[alpha_label] = []
    ...
    for _ in range(n_repeat):
        ...
        by_lang[alpha_label].append(df.groupby('language')['accuracy'].mean())
        by_subj[alpha_label].append(df.groupby('subject')['accuracy'].mean())
        by_subj_lang[alpha_label].append(df.groupby(['subject', 'language'])['accuracy'].mean())
        ...
        by_lang_base[alpha_label].append(df_base.groupby('language')['accuracy'].mean())
        by_subj_base[alpha_label].append(df_base.groupby('subject')['accuracy'].mean())
        by_subj_lang_base[alpha_label].append(df_base.groupby(['subject', 'language'])['accuracy'].mean())
...
# By language
lang_df      = summarize_grouped(by_lang)
lang_base_df = summarize_grouped(by_lang_base)
...
subj_lang_df      = summarize_grouped(by_subj_lang)
subj_lang_base_df = summarize_grouped(by_subj_lang_base)
...
print("\n=== By subject x language (preference-aware) ===")
print(subj_lang_df)
print("\n=== By subject x language (base) ===")
print(subj_lang_base_df)

# Save all to parquet
results_df.to_parquet("router_results.parquet")
results_base_df.to_parquet("router_base_results.parquet")
lang_df.to_parquet("router_results_by_language.parquet")
lang_base_df.to_parquet("router_base_results_by_language.parquet")
subj_df.to_parquet("router_results_by_subject.parquet")
subj_base_df.to_parquet("router_base_results_by_subject.parquet")
subj_lang_df.to_parquet("router_results_by_subject_language.parquet")
subj_lang_base_df.to_parquet("router_base_results_by_subject_language.parquet")